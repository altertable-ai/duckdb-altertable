...
AltertableConnection AltertableConnection::Open(const string &dsn) {
    ...
    if (config.find("port") != config.end()) port = std::stoi(config["port"]);
    if (config.find("user") != config.end()) user = config["user"];
    if (config.find("password") != config.end()) password = config["password"];
    if (config.find("ssl") != config.end()) use_tls = config["ssl"] != "false";
    if (config.find("dbname") != config.end()) catalog = config["dbname"];
    ...
    auto sql_client = std::make_unique<arrow::flight::sql::FlightSqlClient>(std::move(flight_client_result.ValueOrDie()));
