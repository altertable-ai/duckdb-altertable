...
void AltertableSchemaSet::LoadEntries(AltertableTransaction &transaction) {
    ...
    for (int64_t i = 0; i < schemas->length(); ++i) {
        ...
        auto schema = make_shared_ptr<AltertableSchemaEntry>(catalog, info, 
            nullptr, nullptr);
...
optional_ptr<CatalogEntry> AltertableSchemaSet::CreateSchema(AltertableTransaction &transaction, CreateSchemaInfo &info) {
