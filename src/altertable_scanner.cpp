...
struct AltertableGlobalState : public GlobalTableFunctionState {
    ...
    bool TryOpenNewConnection(ClientContext &context, AltertableLocalState &lstate, const AltertableBindData &bind_data);
...
void AltertableScanFunction::PrepareBind(AltertableVersion version, ClientContext &context, AltertableBindData &bind_data,
...
static unique_ptr<FunctionData> AltertableBind(ClientContext &context, TableFunctionBindInput &input,
    ...
    if (!bind_data->catalog_name.empty()) {
        table_ref = "\"" + bind_data->catalog_name + "\".\"" + bind_data->schema_name + "\".\"" + bind_data->table_name + "\"";
    } else {
    ...
    if (!bind_data.sql.empty()) {
        ...
        if (input.column_ids.empty()) {
            ...
            for (auto &col_idx : input.column_ids) {
                if (col_idx == COLUMN_IDENTIFIER_ROW_ID) continue;
                if (!first) query += ", ";
        ...
        if (!bind_data.catalog_name.empty()) {
            query += " FROM \"" + bind_data.catalog_name + "\".\"" + bind_data.schema_name + "\".\"" + bind_data.table_name + "\"";
        } else {
...
AltertableScanFunction::AltertableScanFunction()
    : TableFunction("altertable_scan", {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR}, AltertableScan,
                    AltertableBind, AltertableInitGlobalState, AltertableInitLocalState) {
